...
import { getDtrClient, encodeAasId, mapShellDescriptor } from "../lib/dtrClient.js";
...
router.get("/shells", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const items = Array.isArray(data?.result) ? data.result.map(mapShellDescriptor) : [];
    res.json({
      items,
...
router.get("/shells/:aasId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
...
router.get("/shells/:aasId/raw", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
    const id = encodeAasId(req.params.aasId);
    const { data } = await client.get(`/shell-descriptors/${id}`);
...
router.post("/shells", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
...
router.put("/shells/:aasId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
    const id = encodeAasId(req.params.aasId);
...
router.delete("/shells/:aasId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
    const id = encodeAasId(req.params.aasId);
...
router.post("/lookup", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const client = getDtrClient();
    const assetIds = (req.body?.assetIds ?? []) as Array<{ name: string; value: string }>;
