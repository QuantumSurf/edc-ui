...
import { getEdcClient, withJsonLd, mapNegotiation, type NegotiationMeta } from "../lib/edcClient.js";
...
router.post("/:id/negotiations", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const connectorId = req.params.id;
    const { client } = await resolveConnector(connectorId);
    const response = await client.post("/v3/contractnegotiations/request", withJsonLd(req.body));
...
router.post("/:id/negotiations/start", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const { offerId, assetId, providerDid, dspEndpoint, offerPolicy } = req.body ?? {};

    if (!offerId || !assetId || !dspEndpoint) {
      res.status(400).json({ error: "offerId, assetId, and dspEndpoint are required" });
      return;
    }
    if (typeof dspEndpoint !== "string" || dspEndpoint.length > 2048) {
      res.status(400).json({ error: "dspEndpoint must be a string and within 2048 chars" });
      return;
    }
    ...
    const response = await client.post("/v3/contractnegotiations", contractRequest);
...
router.post("/:id/negotiations/:negId/terminate", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const { reason } = req.body ?? {};
    await client.post(`/v3/contractnegotiations/${req.params.negId}/terminate`, {
      "@context": { "@vocab": "https://w3id.org/edc/v0.0.1/ns/" },
...
router.get("/:id/negotiations/:negId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.get(`/v3/contractnegotiations/${req.params.negId}`);
    res.json(response.data);
  } catch (error) {
    next(error);
  }
...
router.get("/:id/negotiations/:negId/agreement", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.get(`/v3/contractnegotiations/${req.params.negId}/agreement`);
    res.json(response.data);
  } catch (error) {
    next(error);
  }
