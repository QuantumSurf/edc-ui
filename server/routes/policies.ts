...
router.post("/:id/policies", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [polRes, offRes] = await Promise.all([
      ...
      client.post("/v3/contractdefinitions/request", withJsonLd({})).catch(() => ({ data: [] })),
    ]);
    const mapped = Array.isArray(polRes.data) ? polRes.data.map(mapPolicy) : polRes.data;
    ...
    if (Array.isArray(mapped) && Array.isArray(offRes.data)) {
      ...
      for (const off of offRes.data) {
        ...
        const contract = off["contractPolicyId"] ?? off["edc:contractPolicyId"] ?? "";
        if (access) counts.set(access as string, (counts.get(access as string) ?? 0) + 1);
        if (contract && contract !== access) counts.set(contract as string, (counts.get(contract as string) ?? 0) + 1);
      }
      for (const p of mapped) {
        (p as { offers: number }).offers = counts.get((p as { id: string }).id) ?? 0;
      }
    }
...
router.post("/:id/policies/create", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const edcBody = {
      ...
      "policy": {
        ...
        "odrl:permission": [{
          ...
          ...(constraints?.length ? {
            "odrl:constraint": constraints.map((c: { leftOperand: string; operator: string; rightOperand: string }) => ({
...
router.get("/:id/policies/:policyId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.get(`/v3/policydefinitions/${req.params.policyId}`);
...
router.put("/:id/policies/:policyId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.put(`/v3/policydefinitions/${req.params.policyId}`, withJsonLd(req.body));
    res.json(response.data);
  } catch (error) {
    next(error);
  }
...
router.delete("/:id/policies/:policyId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.delete(`/v3/policydefinitions/${req.params.policyId}`);
    res.json(response.data);
  } catch (error) {
    next(error);
  }
