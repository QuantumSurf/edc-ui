...
import { verifyPassword, dummyVerify, signToken, type Role } from "../lib/auth.js";
...
router.post("/login", loginRateLimit, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (typeof tenantId !== "string" || typeof password !== "string") {
      res.status(400).json({ error: "tenantId and password are required strings" });
      return;
    }
    if (tenantId.length > MAX_TENANT_LEN || password.length > MAX_PASSWORD_LEN || password.length === 0) {
    ...
    const u = rows[0] as {
      ...
      tenant_id: string; tenant_name: string | null; tenant_bpn: string | null;
    };
    ...
    const token = signToken({ id: u.id, email: u.email, role: u.role, name: u.name, tenantId: u.tenant_id ?? undefined });
    res.json({
      token,
      user: {
...
router.get("/me", requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const u = (req as Request & { user?: { id: string; email: string; role: Role; name?: string; tenantId?: string } }).user;
    if (!u) {
