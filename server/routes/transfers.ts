...
import { getEdcClient, withJsonLd, mapTransfer, type TransferMeta } from "../lib/edcClient.js";
...
async function getMetaMap(connectorId: string): Promise<Map<string, TransferMeta & { hidden: boolean }>> {
  ...
  for (const r of rows) {
    map.set(r.transfer_id, {
      ...
      fetch_duration_ms: r.fetch_duration_ms != null ? Number(r.fetch_duration_ms) : null,
      hidden: r.hidden,
    });
  }
  return map;
...
router.post("/:id/transfers", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const mapped = Array.isArray(response.data)
      ? response.data
          .map((raw: Record<string, unknown>) => {
            ...
            return { ...mapTransfer(raw, metaMap.get(id)), _hidden: metaMap.get(id)?.hidden ?? false };
          })
          .filter((t) => !t._hidden)
          .map(({ _hidden: _, ...t }) => t)
      : response.data;
...
router.post("/:id/transfers/start", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const connectorId = req.params.id;
    const { client } = await resolveConnector(connectorId);
    const { agreementId, counterPartyAddress, assetId, dataSink } = req.body ?? {};

    if (!agreementId) { res.status(400).json({ error: "agreementId is required" }); return; }
    if (!counterPartyAddress) { res.status(400).json({ error: "counterPartyAddress (provider DSP endpoint) is required" }); return; }
    if (typeof counterPartyAddress !== "string" || counterPartyAddress.length > 2048) {
      res.status(400).json({ error: "counterPartyAddress must be a string and within 2048 chars" });
      return;
    }
    const tpSsrfErr = validateDspEndpoint(counterPartyAddress);
    if (tpSsrfErr) {
      res.status(400).json({ error: `Rejected counterPartyAddress: ${tpSsrfErr}` });
      return;
    }
    ...
    const transferRequest: Record<string, unknown> = {
      ...
      "dataDestination": isProxy
        ...
        : { "@type": "DataAddress", "type": "HttpData", "baseUrl": dataSink?.endpoint ?? "" },
    };
    if (assetId) transferRequest["assetId"] = assetId;

    const response = await client.post("/v3/transferprocesses", transferRequest);
...
router.get("/:id/transfers/:tpId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
    const response = await client.get(`/v3/transferprocesses/${req.params.tpId}`);
    res.json(response.data);
  } catch (error) {
    next(error);
  }
...
router.post("/:id/transfers/:tpId/fetch", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!endpoint || !token) {
      res.status(404).json({ error: "EDR not found or expired for this transfer" });
      return;
    }
    ...
    const contentType = (dataRes.headers["content-type"] as string) ?? "application/octet-stream";
...
router.delete("/:id/transfers", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const connectorId = req.params.id;
    const { client } = await resolveConnector(connectorId);

    // 1. 현재 전송 목록 조회
    const response = await client.post("/v3/transferprocesses/request", withJsonLd({}));
    const allTransfers: Array<{ id: string; state: string }> = Array.isArray(response.data)
...
router.post("/:id/transfers/:tpId/complete", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { tpId } = req.params;
    const connectorId = req.params.id;
    const { client } = await resolveConnector(connectorId);
    ...
    const alreadyFetched = rows.length > 0 && rows[0].fetch_duration_ms != null;
...
router.post("/:id/transfers/:tpId/terminate", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { tpId } = req.params;
    const connectorId = req.params.id;
    const { client } = await resolveConnector(connectorId);
