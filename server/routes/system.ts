...
async function probeEdcVersion(): Promise<string | null> {
  try {
    ...
    const c = conns[0];
    const client = getEdcClient(c.id, { managementUrl: c.managementUrl, apiKey: c.apiKey });
    ...
    if (Array.isArray(data) && data.length) {
      const ver = (data[0]?.version ?? data[0]?.["edc.runtime.version"]) as string | undefined;
      if (ver) return `v${ver}`;
...
async function getEdcRuntimeVersion(): Promise<string> {
  ...
  const now = Date.now();
  if (edcVersionCache && edcVersionCache.expiresAt > now) return edcVersionCache.value;
...
console.log(`[system] connectorHub=${connectorHubVersion} (source=${connectorHubVersionSource})`);

/* ─── /info ───────────────────────────────────────────────────────── */
router.get("/info", async (_req: Request, res: Response) => {
  const nodeEnv = process.env.NODE_ENV ?? "development";
  const environment = (process.env.APP_ENV ?? (nodeEnv === "production" ? "PROD" : "DEV")).toUpperCase();
