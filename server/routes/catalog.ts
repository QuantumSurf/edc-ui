...
function mapCatalogResponse(data: Record<string, unknown>, fallbackDspEndpoint = ""): CatalogOffer[] {
  ...
  else if (raw && typeof raw === "object") datasets = [raw as Record<string, unknown>];

  // Extract providerDid from catalog root participantId
  const providerDid = (data["participantId"] ?? data["edc:participantId"] ?? "") as string;
  ...
  if (rootService) {
    ...
    const ep = ((svc as Record<string, unknown>)?.["endpointURL"] ?? (svc as Record<string, unknown>)?.["dcat:endpointURL"] ?? "") as string;
    if (ep) rootDspEndpoint = ep;
  }

  return datasets.map((ds) => {
    const assetId = (ds["@id"] ?? ds["edc:id"] ?? ds["id"] ?? "unknown") as string;
    const name = (ds["name"] ?? ds["edc:name"] ?? ds["rdfs:label"] ?? assetId) as string;
    const type = (ds["edc:type"] ?? (ds["dct:type"] as Record<string, unknown>)?.["@id"] ?? ds["type"] ?? "Asset") as string;

    // Extract policy IDs from odrl:hasPolicy
    const rawPolicy = ds["odrl:hasPolicy"] ?? ds["hasPolicy"] ?? [];
    const policies: Record<string, unknown>[] = Array.isArray(rawPolicy) ? rawPolicy : [rawPolicy];
    const pols = policies
    ...
    if (dist) {
      ...
      src = ((d as Record<string, unknown>)?.["dcat:accessURL"] ?? (d as Record<string, unknown>)?.["dcat:downloadURL"] ?? "") as string;
      // Extract DSP endpoint from accessService
      const accessService = (d as Record<string, unknown>)?.["accessService"] ?? (d as Record<string, unknown>)?.["dcat:accessService"];
      if (accessService) {
        const svc = Array.isArray(accessService) ? accessService[0] : accessService;
        const ep = (svc as Record<string, unknown>)?.["endpointURL"] ?? (svc as Record<string, unknown>)?.["dcat:endpointURL"] ?? "";
        if (ep) dspEndpoint = ep as string;
      }
    }

    return { name, type, src, pols, offerId, offerPolicy, assetId, dspEndpoint, providerDid };
  });
}

// POST /:id/catalog — proxy to POST /v3/catalog/request
// 인증된 모든 역할이 카탈로그 조회 가능. SSRF 방지를 위해 dspEndpoint 사전 검증.
router.post("/:id/catalog", requireRole("admin", "operator", "viewer"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { dspEndpoint, counterPartyId } = req.body ?? {};

    if (!dspEndpoint || typeof dspEndpoint !== "string") {
      res.status(400).json({ error: "dspEndpoint is required and must be a string" });
      return;
    }
    if (!counterPartyId || typeof counterPartyId !== "string") {
      res.status(400).json({ error: "counterPartyId is required and must be a string" });
      return;
    }
    ...
    const response = await client.post("/v3/catalog/request", catalogRequest, { timeout: 60_000 });

    // Map DCAT response to CatalogOffer[] (queried DSP endpoint as fallback)
    const offers = mapCatalogResponse(response.data ?? {}, normalizedDspEndpoint);
    res.json(offers);
  } catch (error) {
    next(error);
  }
