...
router.post("/:id/assets", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const mapped = Array.isArray(assetRes.data) ? assetRes.data.map(mapAsset) : assetRes.data;
    ...
    if (Array.isArray(mapped) && Array.isArray(offRes.data)) {
      ...
      for (const off of offRes.data) {
        ...
        if (selector) {
          ...
          const assetId = (sel as Record<string, unknown>)?.["operandRight"] as string
            ?? (sel as Record<string, unknown>)?.["edc:operandRight"] as string ?? "";
          if (assetId) offeredAssetIds.add(assetId);
        }
      }
      for (const a of mapped) {
        (a as { offered: boolean }).offered = offeredAssetIds.has((a as { id: string }).id);
      }
    }
...
router.post("/:id/assets/create", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
...
router.get("/:id/assets/:assetId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
...
router.put("/:id/assets/:assetId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
...
router.delete("/:id/assets/:assetId", writeGuard, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { client } = await resolveConnector(req.params.id);
