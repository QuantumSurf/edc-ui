// KMX EDC — Express 앱 조립(buildApp)
// index.ts(부트스트랩)와 통합테스트(supertest)가 공유하는 앱 구성. 여기서는 미들웨어·라우트만
// 조립해 반환하고(리슨하지 않음), 서버 리슨·DB 초기화·백그라운드 폴러는 index.ts 가 담당한다.
// 이 모듈은 import 부작용이 없어 테스트에서 안전하게 buildApp() 만 가져다 쓸 수 있다.
...
import { getPool, isSchemaReady } from "./lib/db.js";
...
/** Express 앱을 조립해 반환한다(리슨하지 않음). 부트스트랩(index.ts)과 통합테스트가 공유. */
