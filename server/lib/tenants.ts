...
export async function getTenant(id: string): Promise<Tenant | undefined> {
  const { rows } = await getPool().query<{ id: string; name: string; bpn: string }>(
    `SELECT id, name, bpn FROM tenants WHERE id = $1 LIMIT 1`,
    [id],
  );
  return rows[0];
}
...
export async function isBpnTaken(bpn: string, exceptTenantId: string): Promise<boolean> {
...
export async function updateTenantBpn(id: string, bpn: string): Promise<Tenant | undefined> {
  const { rows } = await getPool().query<{ id: string; name: string; bpn: string }>(
...
export async function getTenantSetting(tenantId: string, key: string): Promise<string> {
...
export async function getTenantSettings(tenantId: string, keys: string[]): Promise<Record<string, string>> {
