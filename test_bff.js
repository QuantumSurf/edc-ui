      ...
      const req = http.request(opts, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            const count = Array.isArray(parsed) ? parsed.length : (parsed.data ? (Array.isArray(parsed.data) ? parsed.data.length : 0) : 0);
            console.log('GET ' + path.split('/').pop() + ' | Status: ' + res.statusCode + ' | Count: ' + count);
          } catch {
            console.log('GET ' + path.split('/').pop() + ' | Status: ' + res.statusCode);
          }
          resolve();
        });
      });
    ...
    const req = http.request(opts, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log('POST /catalog | Status: ' + res.statusCode + ' | Datasets: ' + datasets.length);
        } catch {
