...
const ko = {
  ...
  common: {
    ...
    expand: "펼치기",
    collapse: "접기",
    errorOccurred: "예기치 않은 오류가 발생했습니다.",
    reloadPage: "페이지 새로고침",
  ...
  assets: {
    ...
    s3Region: "S3 리전 (region)",
    s3Bucket: "S3 버킷 (bucketName)",
    s3ObjectName: "객체 키 (objectName)",
    s3Endpoint: "엔드포인트 오버라이드 (endpointOverride)",
    s3EndpointHint: "MinIO 등 S3 호환 스토리지 사용 시에만 입력하세요.",
    s3AccessKeyId: "액세스 키 ID (accessKeyId)",
    s3SecretKey: "시크릿 액세스 키 (secretAccessKey)",
    s3CredentialKeepHint: "비워두면 기존 자격증명을 유지합니다.",
    s3Required: "AmazonS3 자산은 region과 bucketName이 필수입니다.",
    ...
    dataAddressType: "소스 유형",
    proxyPath: "프록시 경로",
    proxyQueryParams: "프록시 쿼리 전달",
    contentTypeLabel: "콘텐츠 타입",
    authCodeLabel: "authCode (Vault 별칭)",
    aasVersionLabel: "AAS 버전",
    aasIdLabel: "AAS ID",
    submodelIdLabel: "서브모델 ID",
  ...
  policies: {
    ...
    bpnGroups: {
      button: "BPN 그룹",
      title: "BPN 그룹 관리",
      subtitle: "BusinessPartnerGroup 정책 제약이 평가하는 BPN→그룹 매핑",
      hint: "그룹 정책(isAnyOf 등)은 여기 등록된 매핑으로 평가됩니다. 미등록 BPN은 그룹 정책에서 항상 거부됩니다.",
      empty: "등록된 그룹이 없습니다.",
      addTitle: "BPN 매핑 추가/수정",
      bpnLabel: "BPN",
      groupsLabel: "그룹 (쉼표로 구분)",
      groupsPlaceholder: "fl-partners, trusted-suppliers",
      save: "저장",
      saved: "BPN 그룹 매핑이 저장되었습니다.",
      deleted: "BPN 그룹 매핑이 삭제되었습니다.",
      deleteConfirm: (bpn: string) =>
        `'${bpn}'의 그룹 매핑을 삭제하시겠습니까?`,
      bpnInvalid: "BPN 형식이 아닙니다 (BPNL + 영숫자 12자리).",
      groupsRequired: "그룹을 1개 이상 입력하세요.",
      memberCount: (n: number) => `${n}개 BPN`,
    },
  ...
  transfers: {
    ...
    bulk: {
      ...
      bucket: "버킷",
      objectKey: "오브젝트 키(선택)",
      endpoint: "엔드포인트(S3 호환·MinIO, 선택)",
      accessKey: "액세스 키",
      secretKey: "시크릿 키",
      destHint:
        "S3/MinIO 목적지를 입력하세요. 비우면 서버 기본값(S3_*)을 사용합니다. MinIO는 엔드포인트에 http://host:9000 형식으로.",
      cancel: "취소",
      canceling: "취소 중...",
      overall: "전체",
      currentFile: "현재 파일",
  ...
  addConnector: {
    ...
    managementUrlRequired: "Management API URL을 입력하세요.",
    registerFailed: "커넥터 등록에 실패했습니다.",
    register: "커넥터 등록",
  },
  ...
  settings: {
    ...
    save: "저장",
    discard: "되돌리기",
    readOnlyNotice: "관리자만 이 설정을 수정할 수 있습니다 (읽기 전용).",
    personalGroup: "개인 환경설정",
    systemGroup: "조직·시스템 설정",
  },
  ...
  notifications: {
    ...
    tabUnread: "안읽음",
    unreadLabel: "읽지 않음",
    clearAllConfirmTitle: "모든 알림 삭제",
    clearAllConfirmDesc: "모든 알림이 영구 삭제됩니다. 되돌릴 수 없습니다.",
  ...
  twins: {
    ...
    discovery: {
      action: "셸 검색",
      title: "specificAssetId 로 셸 검색 (디스커버리)",
      desc: "AAS Registry & Discovery 표준 경로입니다. 자산 식별자(name/value)로 등록된 셸의 AAS ID를 찾습니다 — 목록 텍스트 검색과 달리 레지스트리에 직접 질의합니다.",
      addPair: "조건 추가",
      search: "검색",
      searching: "검색 중...",
      results: (n: number) => `결과 ${n}건`,
      empty: "일치하는 셸이 없습니다.",
      needCriteria: "검색 조건(name/value)을 1개 이상 입력하세요.",
      failed: "디스커버리 검색에 실패했습니다.",
      inList: "목록에 있음",
    },
    msg: {
      deleted: (name: string) => `${name} 삭제됨`,
