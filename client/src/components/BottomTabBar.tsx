...
export default function BottomTabBar() {
  ...
  const connectorOverflow: OverflowItem[] = connector
    ? [
        isProvider
          ? {
              path: `/connectors/${connId}/catalog`,
              label: t.nav.catalog,
              Icon: Search,
            }
          : {
              path: `/connectors/${connId}/assets`,
              label: t.nav.assets,
              Icon: Package,
            },
        {
          path: `/connectors/${connId}/policy`,
          label: t.nav.policies,
          Icon: ShieldCheck,
        },
        {
          path: `/connectors/${connId}/contract`,
  ...
  return (
    <>
      ...
      >
        ...
        >
          <MoreHorizontal
            className={cn("w-5 h-5", moreActive && "stroke-[2.5px]")}
          />
          <span
            className={cn(
              "text-[11px] whitespace-nowrap",
      ...
      <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
        <SheetContent
          side="bottom"
          className="lg:hidden rounded-t-2xl pb-[max(1rem,env(safe-area-inset-bottom))]"
        >
