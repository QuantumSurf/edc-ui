...
export default function PageInfra() {
  const { t } = useI18n();
  const HIKARI_DATA = HIKARI_DATA_RAW.map(d => ({
    ...d,
    name: t.infra[d.key],
  }));
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card
          title={
            <CardTitle badge={<DataSourceBadge mode="demo" />}>
              {t.infra.k8sDeployments}
            </CardTitle>
          }
        >
          <div className="space-y-0">
            {[
              ...
              <div
                key={n}
                className={`flex justify-between items-center py-2 ${i < arr.length - 1 ? "border-b border-border" : ""}`}
              >
                <span className="text-[12px] text-muted-foreground truncate flex-1 mr-2">
                  {n}
                </span>
          ...
          <div className="space-y-0">
            {[
              ...
              <div
                ...
                className={`flex justify-between items-center py-2 ${i < arr.length - 1 ? "border-b border-border" : ""}`}
              >
                <span className="text-[11px] text-muted-foreground truncate flex-1 mr-2">
                  {n}
                </span>
                <Badge
                  variant={s === "up" ? "green" : s === "gray" ? "gray" : "red"}
                >
                  {s === "up"
                    ? t.infra.ok
                    : s === "gray"
        ...
        >
          <div className="space-y-0">
            {[
              ...
              <div
                key={n}
                className={`py-2 ${i < arr.length - 1 ? "border-b border-border" : ""}`}
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[12px] text-muted-foreground">{n}</span>
                  <Badge
                    variant={
                      c === "green" ? "green" : c === "amber" ? "amber" : "red"
                    }
                  >
                    {v}
                  </Badge>
        ...
        <div className="sm:hidden grid grid-cols-4 gap-2 text-center">
          {HIKARI_DATA.map(d => (
            <div key={d.name} className="bg-muted rounded-lg p-2">
              <div
                className="font-display text-lg font-bold kpi-value"
                style={{ color: d.color }}
              >
                {d.value}
              </div>
        ...
        <div className="space-y-2.5">
          {[
            ...
            <div key={v} className="flex items-center gap-3 text-[11px]">
              <span className="mono text-[11px] text-muted-foreground w-44 flex-shrink-0">
                {v}
              </span>
