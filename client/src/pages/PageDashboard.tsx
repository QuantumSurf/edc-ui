...
export default function PageDashboard({ conn, onNav }: PageDashboardProps) {
  ...
  const pieData = useMemo(() => {
    ...
    return Object.entries(counts)
      .filter(([, v]) => v > 0)
      .map(([name, value]) => ({ name, value, color: FSM_COLORS[name] ?? "#94A3B8" }))
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        ...
        <Card
          ...
          actions={
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-blue-500 rounded inline-block" />{t.dashboard.negotiations}</span>
              <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-emerald-500 rounded inline-block" />{t.dashboard.transfers}</span>
            </div>
          }
        >
        ...
        <Card title={<CardTitle icon={<Activity className="w-3.5 h-3.5 text-blue-500" />}><span className="font-bold">{t.dashboard.fsmDistribution}</span></CardTitle>}>
          <div className="flex flex-col items-center">
            ...
            {pieData.length === 0 ? (
              <div className="text-[11px] text-muted-foreground py-2">{t.dashboard.noResults}</div>
            ) : (
