  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        <KpiCard
          icon={
            <Server className="w-[18px] h-[18px] text-blue-600 dark:text-blue-400" />
          }
          iconBg="bg-blue-50 dark:bg-blue-500/10"
          value={kpiVal(kpi?.totalConnectors)}
          label={t.fleet.totalConnectors}
          loading={kpiLoading}
        />
        <KpiCard
          icon={
          ...
          value={kpiVal(kpi?.up)}
          label={t.fleet.healthy}
          valueColor="text-emerald-600 dark:text-emerald-400"
          loading={kpiLoading}
        />
        <KpiCard
          icon={
            <Shield className="w-[18px] h-[18px] text-amber-600 dark:text-amber-400" />
          }
          iconBg="bg-amber-50 dark:bg-amber-500/10"
          value={kpiVal(kpi?.warn)}
          label={t.fleet.warning}
          sub={t.fleet.needsCheck}
      ...
      {connectorsLoading ? (
        ...
        <div className="py-10 flex flex-col items-center gap-4">
          <ListEmpty
            ...
            message={
              <>
                <span className="block text-[14px] font-semibold text-foreground mb-1">
                  {t.fleet.noConnectors}
                </span>
                <span className="block max-w-md mx-auto">
                  {t.fleet.emptyHelp}
                </span>
              </>
            }
  ...
  return (
    <div className="bg-card rounded-xl p-4 text-left hover:shadow-md transition-all group shadow-sm border border-border hover:border-primary/40">
      ...
      <div className="flex items-start justify-between mb-2">
        <button
          onClick={() => onOpen()}
          className="flex items-center gap-2 text-left flex-1 min-w-0 rounded focus:outline-none focus-visible:ring-1 focus-visible:ring-primary"
        >
          <span className="text-[15px] font-semibold text-foreground group-hover:text-primary transition-colors truncate">
            {c.name}
          </span>
        </button>
      ...
      <button
        onClick={() => onOpen()}
        className="block w-full text-left rounded focus:outline-none focus-visible:ring-1 focus-visible:ring-primary"
      >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-w-0">
        ...
        <FormField label={t.addConnector.managementUrl} required>
          <div className="flex gap-2">
            ...
            />
            {testResult === "ok" && (
              <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0 self-center" />
            )}
            {testResult === "fail" && (
              <XCircle className="w-4 h-4 text-red-500 flex-shrink-0 self-center" />
            )}
          </div>
        </FormField>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <FormField label={t.addConnector.apiKey}>
            <input
              ...
              placeholder={t.fleet.apiKeyUnchanged}
              className={inputClass}
            />
          </FormField>
          ...
          <FormField label={t.addConnector.environment}>
            <select
              value={env}
              onChange={e => setEnv(e.target.value as "PROD" | "STG" | "DEV")}
              className={inputClass}
            >
