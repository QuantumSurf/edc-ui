          ...
          {(["all", "registered", "unregistered"] as const).map(f => (
            ...
            >
              {f === "all"
                ? t.assets.filterAll
                : f === "registered"
                  ? t.assets.filterRegistered
                  : t.assets.filterUnregistered}
            </button>
          ))}
        </div>
      </div>
                ...
                {paginatedData.map(a => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 hidden xl:table-cell">
                      {a.sem ? (
                        <span className="text-xs text-foreground truncate block max-w-[250px]">
                          {a.sem}
                        </span>
                      ) : (
      ...
      {detailTarget && (
        <AssetDetailSheet
          ...
          deleteDisabledReason={
            detailTarget.offered ? t.assets.deleteBlockedByOffering : undefined
          }
        />
      )}
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2 flex-wrap pr-8">
            ...
            <Badge
              variant={target.offered ? "green" : "gray"}
              pulse={target.offered}
            >
        ...
        <div className="flex-1 overflow-auto p-6 space-y-5 text-xs">
          <div>
            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1">
              ...
              {t.assets.sectionBasic}
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <InfoCard
                label={t.assets.col.id}
                value={target.id}
                span
                mono
                copyable={target.id}
          ...
          <div>
            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1">
              <ChevronsRight className="w-3.5 h-3.5 text-primary" />
              {t.assets.sectionDataSource}
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <InfoCard
                label={t.assets.sourceType}
                value={target.dataAddressType}
              />
              <InfoCard
                label={t.assets.sourceUrl}
          ...
          <div>
            <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1">
              <ChevronsRight className="w-3.5 h-3.5 text-primary" />
              {t.assets.sectionMeta}
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <InfoCard
                label={t.assets.col.semanticId}
                value={target.sem}
                span
                mono
  ...
  return (
    <div className="bg-card rounded-xl p-3.5 shadow-sm border border-border hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          <span
            className={`w-1.5 h-1.5 rounded-full ${a.offered ? "bg-emerald-500" : "bg-gray-300"}`}
          />
          <span
            className={`text-[11px] ${a.offered ? "text-emerald-700 dark:text-emerald-300" : "text-muted-foreground"}`}
          >
      ...
      {a.sem && (
        <span className="text-xs text-foreground mt-1.5 block truncate">
          {a.sem}
        </span>
      )}
    </div>
...
function AssetWizard({
  ...
  >(
    baseSrc?.customProperties
      ? Object.entries(baseSrc.customProperties).map(([key, value]) => ({
          key,
          value,
        }))
      : []
  ...
  const updateCustomProp = (
    ...
    setCustomProps(arr =>
      arr.map((p, i) => (i === idx ? { ...p, [field]: val } : p))
    );
    markDirty();
  };
  const addCustomProp = () => {
    setCustomProps(arr => [...arr, { key: "", value: "" }]);
    markDirty();
  };
  const removeCustomProp = (idx: number) => {
    setCustomProps(arr => arr.filter((_, i) => i !== idx));
    markDirty();
  };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <FormField
                ...
                required
                hint={isEdit ? t.assets.idImmutable : undefined}
              >
              ...
              <FormField label={t.assets.dctType} required>
                <select
                  value={dctType}
                  onChange={e => {
                    setDctType(e.target.value);
                    markDirty();
                  }}
                  className={inputBase}
                >
                  <option value="cx-taxo:SubmodelBundle">
                    cx-taxo:SubmodelBundle
                  </option>
                  <option value="cx-taxo:DigitalTwinRegistry">
                    cx-taxo:DigitalTwinRegistry
                  </option>
                  ...
                  {customProps.map((p, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <input
                        value={p.key}
                        onChange={e =>
                          updateCustomProp(idx, "key", e.target.value)
                        }
                      ...
                      <input
                        value={p.value}
                        onChange={e =>
                          updateCustomProp(idx, "value", e.target.value)
                        }
