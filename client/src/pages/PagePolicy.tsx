...
const POLICY_TEMPLATES: PolicyTemplate[] = [
  ...
  },
  {
    ...
    action: "use",
    constraints: [
      { leftOperand: "cx-policy:FrameworkAgreement", operator: "odrl:eq", rightOperand: "Traceability:1.0" },
    ],
  },
  {
    ...
    logicOp: "or",
    constraints: [
      { leftOperand: "BusinessPartnerNumber", operator: "odrl:eq", rightOperand: "BPNL000000000BMW" },
      { leftOperand: "BusinessPartnerNumber", operator: "odrl:eq", rightOperand: "BPNL000000000VW" },
    ],
  },
  {
    id: "membership-and-framework",
    label: "Membership + Framework (AND)",
    description: "활성 멤버십 AND Data Exchange Governance 프레임워크 동시 충족.",
    ruleType: "permission",
    action: "use",
    logicOp: "and",
    constraints: [
      { leftOperand: "cx-policy:Membership", operator: "odrl:eq", rightOperand: "active" },
      { leftOperand: "cx-policy:FrameworkAgreement", operator: "odrl:eq", rightOperand: "DataExchangeGovernance:1.0" },
    ],
  },
  {
    ...
    action: "transfer",
    constraints: [
      { leftOperand: "cx-policy:DataDestination", operator: "odrl:in", rightOperand: "CN,RU,KP" },
    ],
  },
  {
    ...
    action: "use",
    constraints: [
      { leftOperand: "cx-policy:UsagePurpose", operator: "odrl:eq", rightOperand: "cx.core.digitalTwinRegistry:1" },
    ],
  },
];
...
const RIGHT_OPERAND_SUGGESTIONS: Record<string, string[]> = {
  ...
  "cx-policy:FrameworkAgreement": ["DataExchangeGovernance:1.0", "Traceability:1.0", "QualityManagement:1.0"],
  "BusinessPartnerNumber": ["BPNL000000000BMW", "BPNL000000000VW"],
  "cx-policy:UsagePurpose": ["cx.core.digitalTwinRegistry:1", "cx.core.industrycore:1"],
};
...
const RULE_TYPES: { value: RuleType; key: "permission" | "prohibition" | "obligation" }[] = [
  { value: "permission", key: "permission" },
  { value: "prohibition", key: "prohibition" },
  { value: "obligation", key: "obligation" },
];

const ACTIONS = ["use", "transfer", "display", "distribute", "derive", "anonymize", "aggregate", "modify"];
...
function parseConstraints(constraint: string): { left: string; op: string; right: string }[] {
...
export default function PagePolicy() {
  ...
  return (
    <>
      ...
      {/* JSON Viewer */}
      {jsonTarget && <PolicyJsonDialog policy={jsonTarget} onClose={() => setJsonTarget(null)} />}
      ...
      {detailTarget && (
        <PolicyDetailSheet
          ...
          onDelete={detailTarget.offers > 0 ? undefined : () => { setDeleteTarget(detailTarget); setDetailTarget(null); }}
          deleteDisabledReason={detailTarget.offers > 0 ? t.policies.deleteBlockedByOffering : undefined}
        />
      )}
...
/* ─── JSON Viewer Dialog ─────────────────────────────────────── */
function PolicyJsonDialog({ policy, onClose }: { policy: Policy; onClose: () => void }) {
  const { t } = useI18n();
  const envelope = {
    ...
    "odrl:permission": [{
      ...
      "odrl:constraint": parseConstraints(policy.constraint).map((c) => ({
        ...
        "odrl:operator": { "@id": c.op?.startsWith("odrl:") ? c.op : `odrl:${c.op || "eq"}` },
...
function EmptyPolicies({ onCreateClick }: { onCreateClick: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-[12px] text-muted-foreground mb-4 max-w-[260px]">{t.policies.emptyDesc}</p>
      <RoleGate permission="resource:write">
        <button
              ...
              {policies.map((p) => {
                ...
                return (
                  <tr
                    ...
                    className={cn("table-row-hover cursor-pointer group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-primary [&>td:first-child]:border-l-2", selectedId === p.id ? "bg-primary/5 [&>td:first-child]:border-l-primary" : "[&>td:first-child]:border-l-transparent")}
                  >
...
function ODRLBuilder({ open, connectorId, existingPolicyIds = [], editTarget, duplicateSource, onDone, onCancel, onDirtyChange }: { open: boolean; connectorId: string; existingPolicyIds?: string[]; editTarget?: Policy | null; duplicateSource?: Policy | null; onDone: () => void; onCancel?: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const initialConstraints: Constraint[] = (() => {
    if (!baseSrc) return [{ leftOperand: "cx-policy:Membership", operator: "odrl:eq", rightOperand: "active" }];
    const parsed = parseConstraints(baseSrc.constraint);
    if (parsed.length === 0) return [{ leftOperand: "cx-policy:Membership", operator: "odrl:eq", rightOperand: "active" }];
    return parsed.map((c) => ({
      leftOperand: c.left || "cx-policy:Membership",
      operator: c.op ? (c.op.startsWith("odrl:") ? c.op : `odrl:${c.op}`) : "odrl:eq",
  ...
  const importFromJson = (): boolean => {
    try {
      ...
      const detectedAction = (first["odrl:action"] ?? first.action ?? "use") as string;
      ...
      const flattenConstraint = (c: Record<string, unknown>) => {
        ...
        for (const lo of LOGIC_OPS) {
          ...
          if (Array.isArray(inner)) {
            ...
            for (const ic of inner) flattenConstraint(ic as Record<string, unknown>);
            return;
          }
        }
        // Plain constraint
        ...
        const op = typeof opRaw === "string" ? opRaw : ((opRaw as Record<string, unknown>)["@id"] ?? "odrl:eq");
        const right = c["odrl:rightOperand"] ?? c.rightOperand ?? "";
        if (left || right) {
          allConstraints.push({
            leftOperand: String(left),
            operator: String(op).startsWith("odrl:") ? String(op) : `odrl:${op}`,
      ...
      for (const perm of rules) {
        ...
        const cons = (permObj["odrl:constraint"] ?? permObj.constraint ?? []) as unknown[];
      ...
      setJsonError(t.policies.jsonInvalid + ": " + (e instanceof Error ? e.message : String(e)));
      return false;
    }
  };
  ...
  const validatePolicyId = (id: string): string | null => {
    ...
    if (!isEdit && existingPolicyIds.includes(id)) return t.policies.idDuplicate;
    return null;
  };

  const addConstraint = () => {
    setConstraints([...constraints, { leftOperand: "cx-policy:Membership", operator: "odrl:eq", rightOperand: "" }]);
    markDirty();
  };

  const updateConstraint = (idx: number, field: keyof Constraint, value: string) => {
  ...
  const removeConstraint = (idx: number) => { setConstraints(constraints.filter((_, i) => i !== idx)); markDirty(); };
  ...
  const handleSave = async () => {
    ...
    if (idErr) { setPolicyIdError(idErr); toast.error(idErr); return; }
    if (constraints.some((c) => !c.rightOperand)) { toast.error(t.policies.rightOperandRequired); return; }
    setSubmitting(true);
    try {
      const payload = { policyId, ruleType, action, logicOp: constraints.length > 1 ? logicOp : undefined, constraints } as Record<string, unknown>;
      if (isEdit) {
      ...
      const msg = (err as { response?: { data?: { error?: string; message?: string } } })?.response?.data?.error
        ...
        || "";
      toast.error(isEdit ? t.policies.updateFailed : t.policies.createFailed, { description: msg || undefined });
      setSubmitting(false);
      return;
    }
    try { await queryClient.refetchQueries({ queryKey: ["policies", connectorId] }); } catch {}
    setSubmitting(false);
    onDone();
  };
  ...
  const jsonImportContent = (
    <div className="space-y-3">
      <p className="text-[12px] text-muted-foreground">{t.policies.jsonImportHint}</p>
      <textarea
        value={jsonText}
      ...
      <div className="flex gap-2">
        <button
          onClick={() => { if (importFromJson()) { setEditorMode("builder"); toast.success(t.policies.jsonImported); } }}
  ...
  const builderContent = (
    <div className="space-y-4">
          ...
          {(["builder", "json"] as const).map((m) => (
            <button
              ...
              className={`text-[11px] px-3 py-1 rounded transition-colors ${
                editorMode === m ? "bg-card text-foreground font-medium shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {m === "builder" ? t.policies.modeBuilder : t.policies.modeJsonImport}
            </button>
          ))}
        </div>
        {editorMode === "builder" && (
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-muted-foreground">{t.policies.template}:</span>
            <select
              defaultValue=""
              onChange={(e) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1 p-0.5 bg-muted/40 rounded-md">
              {LOGIC_OPS.map((lo) => (
                <button
                  key={lo.value}
                  onClick={() => { setLogicOp(lo.value); markDirty(); }}
                  className={`text-[11px] px-2 py-0.5 rounded transition-colors ${
                    logicOp === lo.value ? "bg-card text-foreground font-medium shadow-sm" : "text-muted-foreground"
                  }`}
      ...
      {constraints.map((c, idx) => (
        <div key={idx} className="border border-border rounded-lg p-3 space-y-3 bg-muted/30">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-medium text-muted-foreground">{t.policies.constraintN(idx + 1)}</span>
            <button onClick={() => removeConstraint(idx)} className="text-rose-500 hover:text-rose-700">
          ...
          {(["builder", "json"] as const).map((tb) => (
            <button key={tb} onClick={() => setMobileTab(tb)}
              className={`px-4 py-2 text-[12px] border-b-2 transition-colors -mb-px flex-1 ${
                mobileTab === tb ? "border-primary text-primary font-medium" : "border-transparent text-muted-foreground"
              }`}>
