...
### 현재 baseline (2026-08-07) — 캐시 스탬피드 수정 후

같은 조건(`BASE_URL=http://localhost:3006`, `API_RATE_LIMIT_MAX=200000`, 램프 0→50 VU 4분)에
**응답하지 않는 커넥터 1개를 등록한 상태**로 측정했다(운영에서 커넥터 한 대가 죽은 상황 재현).
**전 임계값 통과(k6 exit 0)**:

| 지표                | 결과                               | 2026-07-22 대비 |
| ------------------- | ---------------------------------- | --------------- |
| `checks`            | 99.99% (112,574/112,580)           | 유사            |
| `http_req_failed`   | 0.00% (3/60,621)                   | 유사            |
| `app_errors` (5xx)  | 0.00% (3/51,960)                   | 유사            |
| `http_req_duration` | p95=17.86ms, p99=32.43ms           | p95 32.8→17.9ms |
| `api_read_latency`  | p95=20.5ms, p99=36.87ms            | p95 40.5→20.5ms |
| `api_read_samples`  | 34,640                             | —               |
| 처리량              | **251.8 req/s** · 8,660 iterations | 248 → 251.8     |

잔여 오류 3건·체크 실패 6건은 램프 초기 **캐시 콜드** 구간에서 죽은 커넥터의 팬아웃
타임아웃(max 5.01s)을 맞은 최초 요청들이다. 이후로는 SWR 이 흡수한다.

#### 이 수정 전에 무슨 일이 있었나 (회귀 재현 기록)

커넥터 1개가 도달 불가일 때 동일 부하에서 **BFF 전체가 무너졌다**:

| 지표                | 수정 전                     | 단일비행만 | 단일비행+SWR    |
| ------------------- | --------------------------- | ---------- | --------------- |
| 처리량              | 81 req/s                    | 53 req/s   | **251.8 req/s** |
| `http_req_duration` | p95=3.01s                   | p95=4.26s  | **p95=17.86ms** |
| `app_errors`        | 1.31% (227)                 | 0.00%      | **0.00%**       |
| 부하 종료 후        | **`/readyz` 503 영구 고착** | 정상       | **정상**        |

원인은 `ttlCache` 의 캐시 스탬피드였다. TTL(5초)이 만료되는 순간 그 시점의 모든 동시 요청이
미스가 되어 각자 커넥터 팬아웃을 열고, 죽은 커넥터의 타임아웃(3~5초)까지 소켓과 이벤트루프를
붙잡아 pg 풀(max 20, 획득 타임아웃 5초)이 고갈 → 해당 커넥터와 무관한 `/api` 전체가 503 이 되고
**부하가 끝나도 회복되지 않았다**(liveness `/healthz` 는 DB 비의존이라 200 → k8s 가 재시작도
안 해주고 NotReady 로 방치). `getOrCompute` 의 single-flight + stale-while-revalidate 로 해결.

### 이전 baseline (2026-07-22) — 레이트리밋 해제, 앱 경로 실측
